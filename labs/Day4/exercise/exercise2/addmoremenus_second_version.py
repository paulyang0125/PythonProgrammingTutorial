...
#restaurantmenu = {'name':"Urban Burger",[{"name"="Veggie Burger", 'description'="Juicy grilled veggie patty with tomato mayo and lettuce",'price'="$7.50", 'course'="Entree",'restaurant'='restaurant1'}]}
...
menuItem1 = MenuItem(name="French Fries", description="with garlic and parmesan",
                     price=2.49, course="Appetizer", restaurant=restaurant1)
...
menuItem2 = MenuItem(name="French Fries", description="with garlic only",
...
menuItem3 = MenuItem(name="Mom's Spaghetti", description="Spaghetti with some incredible tomato sauce made by mom",
                     price=3.5, course="Entree", restaurant=restaurant1)
...
menuItem5 = MenuItem(name="Tonkatsu Ramen", description="Noodles in a delicious pork-based broth with a soft-boiled egg",
                     price=3.95, course="Entree", restaurant=restaurant1)

session.add(menuItem5)
session.commit()

menuItem5 = MenuItem(name="French Fries", description="original flavour",
...
menuItem3 = MenuItem(name="Morels on toast (seasonal)", description="Wild morel mushrooms fried in butter, served on herbed toast slices",
                     price=2.95, course="Appetizer", restaurant=restaurant1)
...
menuItem10 = MenuItem(name="Spinach Ice Cream", description="vanilla ice cream made with organic spinach leaves",
                      price=2.95, course="Dessert", restaurant=restaurant1)
...
menuItem2 = MenuItem(name="Cachapa", description="Golden brown, corn-based Venezuelan pancake; usually stuffed with queso telita or queso de mano, and possibly lechon. ",
                     price=6.5, course="Entree", restaurant=restaurant1)
...
menuItem1 = MenuItem(name="Lemon Curd Ice Cream Sandwich", description="Lemon Curd Ice Cream Sandwich on a chocolate macaron with cardamom meringue and cashews",
                     price=1.5, course="Dessert", restaurant=restaurant1)

session.add(menuItem1)
session.commit()

menuItem1 = MenuItem(name="French Fries", description="With delicious sources",
                     price=2.83, course="Appetizer", restaurant=restaurant1)

session.add(menuItem1)
session.commit()

# Menu for 大宅門
restaurant1 = Restaurant(name="大宅門")
session.add(restaurant1)
session.commit()

menuItem1 = MenuItem(name="特製酸梅湯", description="店家自行熬煮的，喝起來甘甜解膩",
                     price=3.15, course="Dessert", restaurant=restaurant1)

session.add(menuItem1)
session.commit

menuItem1 = MenuItem(name="皮蛋擂辣椒", description="小黃瓜絲上頭鋪了滿滿調味過的皮蛋，最上頭再擺上少許辣椒點綴",
...
print("added 11 restaurants and their menu items!")
